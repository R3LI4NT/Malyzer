...
public class AnalizadorEstatico
{
    ...
    private void AnalisisProfundoPE(string ruta, byte[] bytes, ResultadoAnalisisEstatico r)
    {
        try
        {
            ...
            try
            {
                var cert = System.Security.Cryptography.X509Certificates.X509Certificate.CreateFromSignedFile(ruta);
                ...
                r.IndicadoresProfundos.Add(new IndicadorProfundo
                {
                    Tipo = "authenticode",
                    Severidad = "info",
                    Descripcion = $"Firmado digitalmente",
                    Detalle = $"Subject: {cert2.Subject} · Issuer: {cert2.Issuer} · Vence: {cert2.NotAfter:yyyy-MM-dd}"
                });
                if (cert2.NotAfter < DateTime.Now)
                    r.IndicadoresProfundos.Add(new IndicadorProfundo { Tipo = "authenticode", Severidad = "media", Descripcion = "Firma vencida", Detalle = $"Expiró {cert2.NotAfter:yyyy-MM-dd}" });
                if (cert2.Subject == cert2.Issuer)
                    r.IndicadoresProfundos.Add(new IndicadorProfundo { Tipo = "authenticode", Severidad = "media", Descripcion = "Certificado auto-firmado", Detalle = cert2.Subject });
            }
            catch
            {
                r.IndicadoresProfundos.Add(new IndicadorProfundo { Tipo = "authenticode", Severidad = "media", Descripcion = "Sin firma digital", Detalle = "Ejecutables legítimos suelen estar firmados con Authenticode" });
            }
