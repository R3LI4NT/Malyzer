...
public partial class PaginaAnalisisEstatico : Page
{
    ...
    private void RellenarTabFirma(ResultadoAnalisisEstatico r)
    {
        var f = r.FirmaDigital;
        if (f != null && f.EstaFirmado)
        {
            textoFirmaEstado.Text = f.EstadoVerificacion;
            textoFirmaEstado.Foreground = (Brush)Application.Current.Resources[
                f.FirmaValida && f.CadenaConfianzaValida && !f.Vencido && !f.AutoFirmado ? "ColorVerde" : "ColorNaranja"];
            textoFirmaSujeto.Text = f.Sujeto;
            textoFirmaEmisor.Text = f.Emisor;
            textoFirmaEmision.Text = f.FechaEmision?.ToString("yyyy-MM-dd HH:mm:ss") ?? "-";
            textoFirmaVencimiento.Text = (f.FechaVencimiento?.ToString("yyyy-MM-dd HH:mm:ss") ?? "-") + (f.Vencido ? "  (vencido)" : "");
            textoFirmaAlgoritmo.Text = f.Algoritmo;
            textoFirmaHuella.Text = string.IsNullOrEmpty(f.Huella) ? "-" : f.Huella;
            textoFirmaSerie.Text = string.IsNullOrEmpty(f.NumeroSerie) ? "-" : f.NumeroSerie;
            var lineas = new List<string>();
            if (f.AutoFirmado) lineas.Add("⚠ Autofirmado");
            if (!f.CadenaConfianzaValida) lineas.Add("⚠ Cadena de confianza inválida");
            if (f.Vencido) lineas.Add("⚠ Certificado vencido");
            if (!f.FirmaValida) lineas.Add("✗ Firma binaria inválida");
            if (f.FirmaValida && f.CadenaConfianzaValida && !f.AutoFirmado && !f.Vencido) lineas.Add("✓ Firma válida y de confianza");
            if (!string.IsNullOrEmpty(f.MensajeError)) lineas.Add(f.MensajeError);
            textoFirmaChequeo.Text = string.Join("\n", lineas);
        }
        else
        {
            textoFirmaEstado.Text = (f?.EstadoVerificacion ?? "Sin firma digital");
            textoFirmaEstado.Foreground = (Brush)Application.Current.Resources["ColorNaranja"];
            textoFirmaSujeto.Text = "-";
            textoFirmaEmisor.Text = "-";
            textoFirmaEmision.Text = "-";
            textoFirmaVencimiento.Text = "-";
            textoFirmaAlgoritmo.Text = "-";
            textoFirmaHuella.Text = "-";
            textoFirmaSerie.Text = "-";
            textoFirmaChequeo.Text = string.IsNullOrEmpty(f?.MensajeError) ? "—" : f.MensajeError;
        }

        var rh = r.RichHeader;
        if (rh != null && rh.Presente)
        {
            textoRichPresente.Text = "Sí";
            textoRichPresente.Foreground = (Brush)Application.Current.Resources["ColorVerde"];
            textoRichCompilador.Text = string.IsNullOrEmpty(rh.CompiladorEstimado) ? "-" : rh.CompiladorEstimado;
            textoRichChecksum.Text = rh.Checksum;
            gridRichEntradas.ItemsSource = rh.Entradas.Select(e => new
            {
        ...
        {
            textoRichPresente.Text = "No";
            textoRichPresente.Foreground = (Brush)Application.Current.Resources["ColorNaranja"];
            textoRichCompilador.Text = "-";
            textoRichChecksum.Text = "-";
            gridRichEntradas.ItemsSource = null;
        }
    }
